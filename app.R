...
tbl_output <- tbl_targets %>% 
  select(targetId,
   ...
   starts_with("icon"), 
         all_of(d[d %in% colnames(tbl_targets)])) 
...
column_display_names = c("Gene Symbol" = "targetSymbol",
                         "Multimorbidity Count" = "overallMultimorbidityCount",
                         "GenAge" = "icon.GenAge", 
                         "Hallmarks" = "icon.goHallmarks",
                         "Tractability" = "icon.tractability",
                         "TEPs/probes" = "icon.tep_probe",
                         "Safety warning" = "icon.safety_warning")
...
server <- function(input, output, session) {
  ...
  ")
    ...
    output$open_ot <- renderUI({
        d_url <- ot_url()
        tags$a(href = d_url, "See more at Open Targets", target = "_blank")
    })
